...
var ANPIRPSL=1;
var ANPIRSSL=0;
var ANPIS="unescape('&orig='+safeVarClean('kvocity')+','+safeVarClean('kvost')+'&dest='+safeVarClean('tacProp.endCity')+','+safeVarClean('tacProp.endState')+'&dist='+safeVarClean('tacProp.distanceBucket')+'&cat='+safeVar('category')+'&sub='+safeVar('subCategory')).toLowerCase()";
var ANPUF=1;
var ANSID=16730;
var ANTCC;
...
var ANRDF=1;
var ANSCC="unescape(isSocialNetworkShared()+'&orig='+safeVarClean('tacProp.oriCity')+','+safeVarClean('tacProp.oriState')+'&dest='+safeVarClean('tacProp.endCity')+','+safeVarClean('tacProp.endState')+'&dist='+safeVarClean('tacProp.distanceBucket')+'&cat='+safeVar('category')+'&sub='+safeVar('subCategory')+'&brand='+safeVar('brand')+document.referrer+document.location.href+tacLogRef5WithStr(safeVar('category')+':'+safeVar('subCategory')+':'+safeVar('brand'))).toLowerCase()";
var ANTPUD;
var ANVDT=0;
var CCLOOKUP22='AASBZ/apps/listing|AAAIWcat=airport|EAAHXhotel|AARESreal_estate_|AAGLFgolf_courses-|AAABTpharmacies|AAAIWsub=airport|AB#ACWcat=.*restauran|EATVLtravel|EASHPshopping|EADZZbest_western|EAAHXlodging|EAEDUschools|EBDVAtourist_attraction|EBAKRcollege|EADQVgas_station|EAABIapartments|EB#AGP.*doctor|EBAFZhair_salon|EBABXmuseum|EBDXDbank|EBFODgrocery_store|EAAPPclothing|EA#AGS.*dentists|EBHLThealth_service|EAABWtheatres-live|EASHPretail_shops|AAAHXbed_and_breakfast|RAGEO|AAAHXsub=resorts|LAGEP|AADQVgas_station|AAAHYcar_rental|AAAIRauto_repair|AAAIRoil_and_lube|AAABVbookstores|ABAGUauto_dealer|AAAPPretail_apparel|AADVEamusement_parks|ABMOVmovie_theater|ABEDUk12_school|ABAKRcollege|ACFODgrocery_stor|ACAMLoffice_suppli|AAAPPretail_apparel|AASHPdepartment_stores|ABSHPshopping_outlet|ACAIJfloris|BAACWast_food|AAACWpizza|AAACWjapanese|AEACWchi|AAACWitalian|ACACWmexic|AAOLFcampgrounds|ABAHXhotel|ABAKUgolf_course|AAAHSbowling|AASPTstadiums|AADRXcoffee|CAAIXmmuter_rail_stations|ABHLTmedical_center|ABAIQsporting_good|ABDVEamusement_place|ACMOVmovie_theatr|ABADFveterinarian|ABAGUautomobile_dealer|SCAUO-used_ca|ACPFZfinancial_planne|ABAAUbusiness_management_consultant|ADAKMinsurance_age|AAADEwedding|AAPARday_care|AAAHYauto_rental|AASHPgift_shops|ABBTYmanicurist|ABABPhealth_clubs_and_gym|BBAIMome_accessories_and_furnishing|ABPELstereos_and_electronic|ABARTarts_organization|AAACVsports_recreational|BAOLFkiing_centers|ABEQNfurniture_store|ABNLFnight_club|AAELKdest=chandler,az|FDEKFphoenix|FAEKIirvine,ca|FDEJWlosangeles|FDEKZpleasanton|FDELDsacramento|HEEKSndieg|IEELGfrancisc|FAEKHdenver,co|FAEKCwashington,dc|FAELSjacksonville,fl|FDEKGmiami|FDELHorlando|FDEKDtampa|FAEJXatlanta,ga|FAEJZchicago,il|FDEKLelmhurst|FAEKRindianapolis,in|FAELIlouisville,ky|FAEKXbaltimore,md|GAEKKoston,ma|FAELMsouthfield,mi|FAEKJminneapolis,mn|FAELFkansascity,mo|FDEKMstlouis|FAELElasvegas,nv|FAEKPnewark,nj|IAEJYyork,ny|FAEKWcharlotte,nc|FDEKYraleigh|FAELAcincinnati,oh|GDEKVleveland|GDEKTolumbus|FAELOoklahomacity,ok|FAEKNtoronto,on|FAELRportland,or|GAEKAhiladelphia,pa|GDELJittsburgh|FAELQmontreal,qc|FAEKUnashville,tn|FAEKQaustin,tx|FDEKEdallas|FDEKBhouston|FFELBrichards|FDELCsanantonio|HAELTltlakecity,ut|FAELLnorfolk,va|FDELNrichmond|FBEKOseattle,w|FAELPmilwaukee,wi|FAERLatlanticcity,nj|BAELUist=1|FAELV2|FAELW3|FAELX4|AAFIXsocialnetwork:share:facebook:read|TAECM|TAECI:facebook|UAFILgoogleplus|UAECKlifestream|UAEUMtwitter|UAEWTfacebook:like|[AFIQok:recommend|UAFIWstumbleupon|AA#JAKdest=.*,ak&dist=4|FHDZLphoenix,az|FHDZJatlanta,ga|FIJLAlosangeles,c|FKDZIsandiego|ILJSFfrancisc|FHJCHchicago,il|GHDZNolorado,de|FH#JDC.*,dc|FHJMImiami,fl|FKJORorlando|FH#JHW.*,hi|FKDZXhonolulu|FHDZTneworleans,la|FIJBOboston,m|FHEEZminneapolis,mn|FHDZQsaintlouis,mo|GQDZQ|FHJLVlasvegas,nv|FHERMatlanticcity,nj|FH#JNY.*,ny|FHDZPcharlotte,nc|FHDZOportland,or|GHDZMhiladelphia,pa|FHDZVnashville,tn|FHDZUaustin,tx|FKDZWdallas|FKDZHhouston|FKDZRsanantonio|GHDZKeattle,wa|FHDZSwashington,dc|AAGDX&orig=chicago,il|GAGEDdallas,tx|HAGEEetroit,mi|GAGDYhouston,tx|GAGDWlosangeles,ca|GAGDVnewyork,ny|GAGDZphiladelphia,pa|IAGEAoenix,az|GAGECsanantonio,tx|JAGEBdiego,ca|AAEAPbrand=holiday_inn|GBDZZbest_wester|GBEAEcomfort_in|GEEANhampton';
...
var ANBYODO='bk';
var ANTPX='[24]';
function safeVarClean(variable){
var ret;
try{
eval("ret = "+variable+";");
}catch(err){
...
return unescape(ret).replace(/[ #/:.?=_+%&|\-]/gi,"");
}
...
function tacLogRef5WithStr(str){
...
ustr+="ifu="+escape(document.referrer)+"&lstr="+escape(str);
return escape(ustr);
}
